...
The narrow inherited-gate correction is complete. M5 is **not complete and not merge-ready**. Twelve canonical durable route files exist: nine public routes, login/signup, and the unchanged dashboard. The frozen inventory contains 61 durable routes and 91 total surfaces; file presence is not acceptance. Recovery, onboarding, application families, settings, admin, and most overlays/system states remain outstanding. No placeholder product routes have been created to inflate coverage.

The latest landing checkpoint adds seven scene structures, nine integrity-checked still assets and confined motion. Direct desktop/mobile capture review led to an authored phone mosaic and additional torn-edge clearance. The complete M1–M4 baseline remains mandatory. Required film, independent continent geometry, finished scene-5 media and approved legal publication content remain separate acceptance dependencies.
...
### Mobile correction verified

Head `ec448b0b96ceff635d52457725f545852a6bae6d` passed all seven workflows: [M5](https://github.com/Elceo23x4/elceo_land/actions/runs/35917685912), [M4](https://github.com/Elceo23x4/elceo_land/actions/runs/35917685920), [M3](https://github.com/Elceo23x4/elceo_land/actions/runs/35917685869), [M2](https://github.com/Elceo23x4/elceo_land/actions/runs/35917686090), [M1](https://github.com/Elceo23x4/elceo_land/actions/runs/35917685948), [foundation](https://github.com/Elceo23x4/elceo_land/actions/runs/35917685859), and [Vite](https://github.com/Elceo23x4/elceo_land/actions/runs/35917685887). M5 passed 35 browser tests in 33.4 seconds. The updated 390px capture was directly inspected: the last blind-spot label clears the deckle, and all lower mosaic copy has a full-width reading measure. [Original evidence](https://github.com/Elceo23x4/elceo_land/actions/runs/35917685912/artifacts/10775981573).

The existing Vercel status is green for deployment `GBujZrZR8zLu912mo9kriStDH8os` in `elceo-land`. This is the unchanged production-path preview, not evidence of a Next cutover or a deployed M5 candidate. No Vercel configuration was changed. The report/coverage/gap documentation follow-up records the evidence and recovery-source findings only; its exact-head CI outcome belongs in the draft PR status record.

## Mandatory repair-and-hardening checkpoint

The new user instruction pauses page-family expansion until auth-state correctness, responsive-source/provenance reachability, post-warmup resource settling and exact-head aggregate acceptance are empirically proven. Implementation and permanent invariants are documented in [M5_HARDENING_CHECKPOINT.md](M5_HARDENING_CHECKPOINT.md). That document also covers full-bleed scene widths, active public navigation, dynamic-route identity tests and the reusable system-state matrix.

Auth unavailable is no longer reduced to null/acquisition. Empty session response bodies are malformed, strengthening the existing M3 resolver. M1's deliberately unconfigured public auth check now requires rendered unavailable semantics and absence of acquisition; all protected-route, SVG, portal and authority checks remain. No dashboard source/assets, frozen contracts, lockfile or production deployment configuration change. Inherited workflow checks remain intact; added status invalidation prevents reusing aggregate success across required-workflow attempts. Final empirical evidence and exact head are pending this checkpoint's CI run; no new product routes may be inferred from these repairs.


### Hardening remains blocked on empirical resource settling

The detailed A–J checkpoint report is in [M5_HARDENING_CHECKPOINT.md](M5_HARDENING_CHECKPOINT.md). Auth-state correctness, asset source/reachability, active navigation, full-width scenes and route normalization pass; exact-head aggregation correctly fails closed. Resource profiling identified substantial compiler/test-runner allocation, and a CDP driver removes repeated ARIA/assertion-engine work without bypassing real interactions. The original numeric limits remain unchanged. Even then, measured head `d6045e57c5dc6dd14305d6e725bf62af4dd12ebe` fails retained-heap trend: +339,428 warm-to-final bytes, +157,956 late bytes, +37,746 bytes/journey late slope; nodes/listeners flat. It passed 41/42 M5 browser tests, and M1–M4/foundation/build stayed green. The final report follow-up adds an explicit negative reload/absent-scene test; its CI result belongs to its own exact-head evidence.

No auth-family or broad product expansion proceeds from this checkpoint. No threshold increase, further warm-up extension, production configuration change or dashboard modification is made to obtain green. The remaining memory result is an acceptance blocker, not an optional optimization.
